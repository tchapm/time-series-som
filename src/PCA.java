...
public class PCA {
	...
	public static void main(String[] args) throws IOException {
		...
		String inFile = "/Users/tchap/Documents/SOMfiles/alphaParse.txt";
		...
		String outFile2 = "/Users/tchap/Documents/SOMFiles/alphaValuesTrans.txt";
		FileWriter fstream2 = new FileWriter(outFile2);
		BufferedWriter out2 = new BufferedWriter(fstream2);
		out2.write(18 + " " + 500 + "\n");
		for (int i = 0; i < data.get(0).size() ; i++) {
			for (int j = 0; j < data.size(); j++) {
				out2.write(Double.toString(matrixTran[i][j]) + " ");
			}
			out2.write("\n");
		}
		out2.close();
		double[][] matrixSwap = new double[numElements][data.size()];
		double[][] clone = matrixTran.clone();
		int colA, colB;
		colA = 0;colB = 16;
		matrixSwap = swapColumns(clone, colA, colB, numElements, data.size());
		colA = 1;colB = 3;
		matrixSwap = swapColumns(clone, colA, colB, numElements, data.size());
		colA = 2;colB = 12;
		matrixSwap = swapColumns(clone, colA, colB, numElements, data.size());
		...
		colA = 11;colB = 15;
		matrixSwap = swapColumns(clone, colA, colB, numElements, data.size());
		String outFile3 = "/Users/tchap/Documents/SOMFiles/alphaValuesSwap.txt";
		FileWriter fstream3 = new FileWriter(outFile3);
		BufferedWriter out3 = new BufferedWriter(fstream3);
		out3.write(18 + " " + 500 + "\n");
		for (int i = 0; i < data.get(0).size() ; i++) {
			for (int j = 0; j < data.size(); j++) {
				out3.write(Double.toString(matrixSwap[i][j]) + " ");
			}
			out3.write("\n");
		}
		out3.close();
		//	  matrix = new double[numElements][data.size()];
		//	  for (int i = 0; i < numElements; i++) {
		//		  for (int j = 0; j < data.size(); j++) {
		//			  List<Double> vec = data.get(j);
		//			  matrix[i][j] = vec.get(i);
		//		  }
		//	  }
		...
		List<PrincipleComponent> mainComponents = pca.getDominantComponentsByPer(0.9);

		int counter = 1;
		bug("Showing top " + 0.9 + " principle components.");
		for (PrincipleComponent pc : mainComponents) {
			bug("Component " + (counter++) + ": " + pc.eigenValue);
		}
		...
		String outFile = "/Users/tchap/Documents/alphaValuesPoop.txt";
		FileWriter fstream = new FileWriter(outFile);
		BufferedWriter out = new BufferedWriter(fstream);

		double[][] pcaMatrix = features.getArray();
		out.write(numComponents + " " + k + "\n");
		for(int i=0;i<pcaMatrix.length;i++){
			for(int j=0;j<pcaMatrix[0].length;j++){
				System.out.print(pcaMatrix[i][j] + " ");
				out.write(Double.toString(pcaMatrix[i][j]) + " ");
			}
			System.out.println();
			out.write("\n");
		}
		...
		//
		//	  PrintWriter printOut = new PrintWriter(output, false);
		//	  bug("2Transformed data into PCA-space (k=" + k + ") :");
		//	  xformedData.transpose().print(8, 5);
		//	  xformedData.transpose().print(printOut, 5, 5);
		//	  
	...
	private static double[][] swapColumns(double[][] matSwap, int colA, int colB,
			int numElements, int size) {
//		double[][] matrixSwap = new double[numElements][size];
		double tempA;
		for(int j=0;j<size;j++){
			System.out.println(matSwap[colA][j] + " " + matSwap[colB][j]);
			tempA = matSwap[colA][j];
			matSwap[colA][j] = matSwap[colB][j];
			matSwap[colB][j] = tempA;
//			System.out.println(matSwap[colA][j] + " " + matSwap[colB][j]);
		}
		return matSwap;
	}
	...
	private List<PrincipleComponent> getDominantComponentsByPer(double k) {
		List<PrincipleComponent> ret = new ArrayList<PrincipleComponent>();
		Iterator<PrincipleComponent> it = principleComponents.iterator();
		double sumE = 0.0, totalComp = 0.0;
		while(it.hasNext()){
			sumE += it.next().eigenValue;
		}
		it = principleComponents.iterator();
		int n = 0;
		while(it.hasNext() && totalComp<sumE*0.9){
			n++;
			totalComp+=it.next().eigenValue;
		}
		System.out.println("Number of top components " + n);
		int count = 0;
		for (PrincipleComponent pc : principleComponents) {
			ret.add(pc);
			count++;
