...
public class MagweneMethod {
	...
	private PQNode runThroughBackbone(ArrayList<DataPoint> dataForPoints, Vector<Integer> diamPath, PQNode pqNode, int startPt, int endPt) throws IOException {
		...
		for(PQNode newNode : pqNode.getNodeChildren()){
			...
			for(int i=0; i<distMat.length; i++){
				if(distMat[dp.getName()][i]==1 && !dataForPoints.get(i).isInTree()){
					ArrayList<DataPoint> tempDataPts = makeBranchedTree(mst.getPredMatrix(), dp.getName(), i);
	...
	public static int getLocInVec(int pointName, Vector<Integer> diamPath, ArrayList<DataPoint> dataPoints){
		for(int i=0; i<diamPath.size(); i++){
			DataPoint dp = dataPoints.get(diamPath.get(i));
			if(dp.getName()==pointName){
				return i;
			}
		}
		return -1;
	...
	private static int[] findBackbone(ArrayList<DataPoint> dataPoints, Vector<Integer> path) {
		int start=-1, end=-1;
		int j = path.size()-1;
	...
	public static MinSpanTree makeMST(ArrayList<DataPoint> theDataPts) throws IOException{
	...
	private ArrayList<DataPoint> makeBranchedTree(int[][] predMatrix, int indecPt, int frstBranch) {
		ArrayList<DataPoint> tempTreePoints = new ArrayList<DataPoint>();
